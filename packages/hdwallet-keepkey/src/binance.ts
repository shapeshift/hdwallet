...
export async function binanceSignTx (transport: KeepKeyTransport, msg: Core.BinanceSignTx): Promise<Core.BinanceSignedTx> {
    return transport.lockDuring(async () => {
        ...
        let resp = await transport.call(MessageType.MESSAGETYPE_BINANCESIGNTX, signTx, Core.LONG_TIMEOUT, /*omitLock=*/true)
        ...
        //sent tx to device
        resp = await transport.call(MessageType.MESSAGETYPE_BINANCETRANSFERMSG, send, Core.LONG_TIMEOUT, /*omitLock=*/true)
