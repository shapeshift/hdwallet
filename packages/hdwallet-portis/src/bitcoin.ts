...
export function describeUTXOPath(
  ...
  if (!btcSupportsCoin(coin)) return unknown;
  if (!btcSupportsScriptType(coin, scriptType)) return unknown;
...
export function verifyScriptTypePurpose(scriptType: core.BTCScriptType, purpose: number): boolean {
  return (
    (purpose === 0x80000000 + 44 && scriptType === core.BTCScriptType.Legacy) ||
    (purpose === 0x80000000 + 49 && scriptType === core.BTCScriptType.Segwit) ||
    (purpose === 0x80000000 + 84 && scriptType === core.BTCScriptType.SegwitNative)
  );
}
