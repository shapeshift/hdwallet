...
export async function ethGetPublicKeys(
  ...
  for (const getPublicKey of msg) {
    ...
    if (
      !addressNList.includes(0x80000000 + 44, 0) ||
      !addressNList.includes(0x80000000 + 60, 1)
    ) {
      xpubs.push(null);
      continue;
    }
...
export async function ethSignMessage(
  ...
  const addressRes = await transport.call(
    "Eth",
    "getAddress",
    bip32path,
    false
  ...
  handleError(
    addressRes,
    transport,
    "Unable to obtain ETH address from Ledger."
  );
...
export async function ethVerifyMessage(
  ...
  return (
    core.stripHexPrefixAndLower(msg.address) ===
