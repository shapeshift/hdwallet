import { Keyring, Events, FirmwareUpdateRequired } from '@shapeshift/hdwallet-core';
...
export class ChromeUSBKeyring {
  ...
  constructor (keyring: Keyring) {
    ...
    if(!c.usb) throw new Error('ChromeUSB is not available in this process. This package is intended for Chrome apps and extensions.')
  ...
  protected handleDisconnectKeepKey (device: USBDevice): void {
    this.keyring.remove(device.serialNumber)
