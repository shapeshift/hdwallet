import { Number as Num, InstanceOf, Object as Obj, Literal, Static, Unknown, String as Str, Never, Runtype, Result } from "funtypes";
...
export const integer = Num.withConstraint(x => Number.isSafeInteger(x) || `expected ${x} to be an integer`, {name: "Integer"});
...
const uint16 = Num.withConstraint(x => x === ((x & 0xffff) >>> 0) || `expected ${x} to be an unsigned 16-bit integer`, {name: "Uint16"});
export type Uint16 = Static<typeof uint16>;
export const Uint16: typeof uint16 = uint16;

const uint8 = Num.withConstraint(x => x === ((x & 0xff) >>> 0) || `expected ${x} to be an unsigned 8-bit integer`, {name: "Uint8"});
...
function boundedUintBase<T extends NonNegativeInteger>(max: T) {
    return Object.assign(Uint.withConstraint(
        x => x <= max || `expected ${x} to be less than or equal to ${max}`,
...
function boundedStringBase<T extends RegExp>(regex: T) {
    return Object.assign(Str.withConstraint(
        x => regex.test(x) || `expected ${x} to match regex ${regex}`,
