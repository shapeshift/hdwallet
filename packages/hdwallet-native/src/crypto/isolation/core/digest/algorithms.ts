...
export function _initializeAlgorithms(register: <N extends AlgorithmName>(name: N, fn: Algorithm<N>) => void) {
    ...
    try {
        ...
        register("hash160", (x): any => crypto.createHash("ripemd160").update(crypto.createHash("sha256").update(x).digest()).digest());
        register("sha256", (x): any => crypto.createHash("sha256").update(x).digest());
        register("hash256", (x): any => crypto.createHash("sha256").update(crypto.createHash("sha256").update(x).digest()).digest());
