...
// This can't be inline (or use generic type bounds) because it needs to distributed over the members of a union type.
type unverifiedDigestInner<N> = N extends (keyof typeof AlgorithmLength) ? (ByteArray<typeof AlgorithmLength[N]> & {preimage: ByteArray, algorithm: N}) : never;
...
function digestBase(name?: AlgorithmName) {
    return UnverifiedDigest(name).withConstraint(
        x => ByteArray.equal(x, Algorithms[x.algorithm](x.preimage)) || `expected ${x} to equal the ${x.algorithm} digest of ${x.preimage}`,
        {name: `Digest(${name})`},
    );
}
