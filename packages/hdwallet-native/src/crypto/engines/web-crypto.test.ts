...
describe("WebCryptoEngine JavaScript", () => {
  ...
  it("should decrypt what it encrypts", async () => {
    ...
    const scopedEngine = new WebCryptoEngine();

    const encrypted = await engine.encrypt(data, key, iv);

    const decrypted = utils.fromBufferToUtf8(await scopedEngine.decrypt(encrypted, key, iv));
  ...
  it("should generate random bytes", async () => {
    ...
    const sum = typedArray.reduce((previous, value) => previous + value, 0);
