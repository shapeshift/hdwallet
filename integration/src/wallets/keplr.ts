...
export function createInfo(): core.HDWalletInfo {
  return new keplr.KeplrHDWalletInfo();
}
...
export async function createWallet(): Promise<core.HDWallet> {
  ...
  const wallet = new keplr.KeplrHDWallet(provider);
  await wallet.initialize();
  return wallet;
}

export function selfTest(get: () => core.HDWallet): void {
  let wallet: keplr.KeplrHDWallet;

  beforeAll(async () => {
    const w = get() as keplr.KeplrHDWallet;

    if (keplr.isKeplr(w) && !core.supportsBTC(w) && core.supportsCosmos(w) && core.supportsOsmosis(w)) {
      wallet = w;
    } else {
      throw new Error("Wallet is not a Keplr");
    }
  });

  it("supports Cosmos mainnet", async () => {
    if (!wallet) return;
    expect(await wallet.cosmosSupportsNetwork()).toEqual(true);
  });

  it("supports Osmosis mainnet", async () => {
    if (!wallet) return;
    expect(await wallet.osmosisSupportsNetwork()).toEqual(true);
  });
  ...
  it("does not support Native ShapeShift", async () => {
    if (!wallet) return;
    expect(wallet.cosmosSupportsNativeShapeShift()).toEqual(false);
  });

  it("does not support Secure Transfer", async () => {
    if (!wallet) return;
    expect(await wallet.cosmosSupportsSecureTransfer()).toEqual(false);
  });
